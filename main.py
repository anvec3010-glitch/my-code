...
# === CONFIGURATION (UPDATED FOR THREE BOTS) ===
OWNER_ID = 673854499 # <<< REPLACE WITH YOUR ID
...
# Define configurations for all bots
# IMPORTANT: Each bot MUST have a unique token and a unique db_file.
BOT_CONFIGS = [
    {
        "name": "BotAlpha",
        "token": "8419731392:AAHhyWYc_eBpb50qNH4Sx0feJr9UkWyOUQA", # <<< BOT 1 TOKEN (REPLACE)
        "db_file": "group_data_alpha.db" # UNIQUE DATABASE FILE 1
    },
    {
        "name": "BotBeta",
        "token": "8374636357:AAFfnTJxR7P33lpsPHoOjclZTR1igKhXaNw", # <<< BOT 2 TOKEN (REPLACE)
        "db_file": "group_data_beta.db" # UNIQUE DATABASE FILE 2
    },
    {
        "name": "BotGamma",
        "token": "YOUR_THIRD_BOT_TOKEN_HERE", # <<< BOT 3 TOKEN (MUST REPLACE)
        "db_file": "group_data_gamma.db" # UNIQUE DATABASE FILE 3
    }
]
...
async def set_text_only_permissions(chat: Chat):
    permissions = ChatPermissions(
        can_send_messages=True, can_add_web_page_previews=True, can_send_audios=False,
...
async def set_tracking_phase_permissions(chat: Chat):
    permissions = ChatPermissions(
        can_send_messages=True, can_add_web_page_previews=True, can_send_videos=True,
        can_send_other_messages=True, can_send_audios=False, can_send_documents=False,
        can_send_photos=False, can_send_video_notes=False, can_send_voice_notes=False,
        can_send_polls=False, can_change_info=False, can_invite_users=True, can_pin_messages=False )
    await chat.set_permissions(permissions)
    logger.info(f"Set permissions to TRACKING (Text, Video, GIF) for chat {chat.id}")
...
async def _run_tracking(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        sent_message = await context.bot.send_message(chat_id=chat_id, text=message_text, disable_web_page_preview=False)
        await context.bot.pin_chat_message(chat_id=chat_id, message_id=sent_message.message_id)
    except BadRequest as e:
...
async def _run_muteall(chat_id: int, context: ContextTypes.DEFAULT_TYPE, duration_days: int, is_automated: bool = False):
    ...
    final_users_to_mute = (unsafe_users.union(sr_users)) - whitelisted_users
...
async def _run_close_session(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Job names must be unique per bot, but here we only clear this bot's jobs.
...
async def _send_reminder_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        sent_message = await context.bot.send_message(chat_id=chat_id, text=full_text, parse_mode=ParseMode.HTML)
        await context.bot.pin_chat_message(chat_id=chat_id, message_id=sent_message.message_id)
...
def _generate_unsafe_list_message(chat_id: int, context: ContextTypes.DEFAULT_TYPE) -> str:
    c = get_db_cursor(context)
    c.execute("SELECT DISTINCT telegram_user FROM links WHERE chat_id = ?", (chat_id,))
    all_users = {u[0] for u in c.fetchall()}
    c.execute("SELECT telegram_user FROM status WHERE chat_id = ? AND completed=1", (chat_id,))
    ...
    for tg_user in sorted(list(unsafe_users)):
        ...
        if main and main[0]:
            ...
            list_counter += 1
    if list_counter == 1 and all_users: # If users posted but links were invalid/missing
        return f"{to_sans_serif_monospace('UNSAFE USERS')}\n━━━━━━━━━━━━━━━\nCould not retrieve link details for these {len(unsafe_users)} user(s)."
    elif list_counter == 1:
...
async def unsafe(update: Update, context: ContextTypes.DEFAULT_TYPE):
    effective_chat_id = get_effective_chat_id(update.effective_chat.id, context)
    message_to_send = _generate_unsafe_list_message(effective_chat_id, context)
...
async def list_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not all_users:
        ...
        return
    msg = f"{to_sans_serif_monospace('USERS LIST')}\n"
    msg += "━━━━━━━━━━━━━━━\n"
    list_counter = 1
...
async def clean_chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try: count = int(context.args[0]) if context.args else 100
    except (ValueError, IndexError): count = 100
    chat_id = update.effective_chat.id
...
def run_single_bot_instance(config: dict):
    ...
    if "YOUR_THIRD_BOT_TOKEN_HERE" in bot_token or "YOUR_SECOND_BOT_TOKEN" in bot_token or "7514089447:AAGLaUJGiGlpRl5BIunMZzbuMU_a_T2TNso" == bot_token or not bot_token or len(bot_token) < 10:
        logger.error(f"FATAL: Token for {bot_name} not set or is a placeholder. Skipping setup.")
        return
    ...
    for chat_id, schedule in SCHEDULED_CHATS.items():
        ...
        for time_str in schedule['times']:
            try:
                ...
                # Ensure job name is unique and includes the bot's name if necessary for debugging
    ...
    try:
        # NOTE: This is a blocking call and must run in a separate thread/process for each bot.
...
def main():
    """Starts all configured bots concurrently using threading."""
    threads = []
    
    # Filter out bots with placeholder or empty tokens
    active_configs = [
        c for c in BOT_CONFIGS 
        # Check for placeholder strings and bad tokens
        if c['token'] and "YOUR_SECOND_BOT_TOKEN" not in c['token'] and "YOUR_THIRD_BOT_TOKEN_HERE" not in c['token'] and "7514089447:AAGLaUJGiGlpRl5BIunMZzbuMU_a_T2TNso" != c['token'] and len(c['token']) > 10
    ]

    if not active_configs:
        logger.error("No valid bot configurations found. Please check BOT_CONFIGS.")
        return

    for config in active_configs:
        # Create a new thread for each bot instance
        thread = threading.Thread(target=run_single_bot_instance, args=(config,), name=config['name'])
        threads.append(thread)
        thread.start()

    logger.info(f"Started {len(active_configs)} bot threads. Use Ctrl+C to stop all.")
    
    # Wait for all threads to complete (keeps the main script alive)
